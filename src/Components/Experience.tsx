    ...
    <div className='mt-23 flex flex-col'>
        ...
        <div className='mt-10 ml-40 mr-40'>
            {experience.map((exp, index) => (
                <div key={index} className='flex mb-5 bg-gray-900 gap-5 p-4 rounded-lg relative'>
                  <span className='absolute left-[-24px] top-5 z-10 rounded-full h-3 w-3 bg-purple-500'></span>
                  <span className={`absolute left-[-20px] rounded-full top-10 ${exp.stipend ? "h-44":""} w-1 bg-purple-500 transition-all duration-300`}></span>
                  <div>
